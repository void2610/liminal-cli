...
- `--force` — 中断された実行の残骸 (サーバ側が Running のまま残った状態) を無視して開始する。
...
- 実行開始 (`POST /tests/run`) も同様に、接続断や**本文が空の応答**を最大 30 秒まで再試行する。
  パッケージを更新した直後は DomainReload 中で空応答になりやすく、ここで諦めると
  「更新してすぐテストを回す」がほぼ失敗する。409 (既に実行中) は再試行せず即座に扱う。
- `--json` はレスポンスを丸出し。
