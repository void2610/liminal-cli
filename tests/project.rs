...
use common::{cmd, project_config_path, temp_project};
...
fn project_show_は設定と検出パスを表示する() {
    ...
    std::fs::write(project_config_path(tmp.path()), r#"{"port":7613}"#).unwrap();
...
fn set_port_は壊れたJSONに寛容で上書きする() {
    ...
    assert!(
        std::fs::read_to_string(project_config_path(tmp.path()))
            .unwrap()
            .contains("7613")
    );
}
...
fn unset_port_は壊れたJSONを致命扱いにする() {
    ...
    // 上書きされていないこと
    assert_eq!(
        std::fs::read_to_string(project_config_path(tmp.path())).unwrap(),
        "{broken"
    );
}
